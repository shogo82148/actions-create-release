...
test("create a new release with generated release not", async () => {
  const generateReleaseNotes = jest
    .spyOn(github.Client.prototype, "generateReleaseNotes")
    .mockImplementation(
      async (
        params: github.GenerateReleaseNotesParams,
      ): Promise<github.Result<github.GenerateReleaseNotesResponse, github.GitHubError>> => {
        expect(params).toEqual({
          owner: "shogo82148",
          repo: "github-action-test",
          tag_name: "v1.0.0",
